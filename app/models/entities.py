"""SQLAlchemy ORM entity definitions (re-exported from the package __init__)."""
...
class User(Base):
    ...
    # Bumped to revoke every outstanding token for this user (logout,
    # password change).  Tokens carry the version they were minted with
    # as a ``ver`` claim; a mismatch is rejected.
    token_version: Mapped[int] = mapped_column(Integer, default=0)

    # Account / credits. plan is a tier name ("free", ...); phone is
    # optional (null = 未绑定).  Points are split into subscription
    # points (plan_points) and top-up packs (pack_points); the total
    # shown to the user is their sum, so it is never stored.
    # points_expire_at is null when there is no expiry (无有效期).
    plan: Mapped[str] = mapped_column(String(32), default="free")
    phone: Mapped[str | None] = mapped_column(String(32), nullable=True)
    plan_points: Mapped[int] = mapped_column(Integer, default=0)
    pack_points: Mapped[int] = mapped_column(Integer, default=0)
...
class Run(Base):
    ...
    # At most one *running* run per conversation, enforced by the DB (a
    # partial unique index) so the guard survives the check-then-insert
    # race in start_run.  Finished runs (done/error/cancelled) are not
    # indexed, so a conversation can start a new run once its last one
    # ends.
    __table_args__ = (
        Index(
            "uq_one_running_run_per_conversation",
            "conversation_id",
            unique=True,
